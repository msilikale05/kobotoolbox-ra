...
(function () {
  ...
  // ── Header User Avatar ──
  // Replace the account circle icon in the KPI header with the user's avatar
  (function headerAvatar() {
    function injectAvatar() {
      // Find the account menu / user icon in the header
      var selectors = [
        '.account-box__name',
        '.main-header__icon--account',
        '[data-tip="account"]',
        '.header-icons .account-box',
        '.account-box'
      ];
      var found = false;
      selectors.forEach(function (sel) {
        var els = document.querySelectorAll(sel);
        els.forEach(function (el) {
          if (el.querySelector('.ra-header-avatar')) return;
          // Try to get username from the element or nearby text
          var username = '';
          var nameEl = el.querySelector('.account-box__name');
          if (nameEl) {
          ...
          if (!username) return;
          // Find or create an avatar image before the username text
          var img = document.createElement('img');
          img.className = 'ra-header-avatar';
          img.src = window.__raAvatarUrl(username);
          img.style.cssText = 'width:28px;height:28px;border-radius:50%;object-fit:cover;margin-right:6px;vertical-align:middle;';
